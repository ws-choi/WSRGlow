...
from src.datamodules.components.sr_bianarizer import VFSRBinarizer, VFSRBinarizer_2PR
from src.datamodules.components.sr_dataset import SRDataset, SRDataset_2PR
...
class SRDataModule_2PR(LightningDataModule):
    ...
    def prepare_data(self):
        target_dir = Path(self.data_dir).joinpath('{}_to_{}'.format(self.target_rate, self.resampled_rate))
        if not os.path.exists(target_dir):
            self.binarizer.process()

    def setup(self, stage: Optional[str] = None):
        self.data_train = SRDataset_2PR(shuffle=True,
                                        sort_by_len=self.sort_by_len,
                                        max_frames=self.max_frames,
                                        prefix='train',
                                        binary_data_dir=self.data_dir,
